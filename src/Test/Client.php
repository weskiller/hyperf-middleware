 ...
 */
namespace Weskiller\HyperfMiddleware\Test;
...
use Weskiller\HyperfMiddleware\Http\Dispatcher;
use Weskiller\HyperfMiddleware\Http\Server;
...
class Client extends Server
{
    ...
    {
        parent::__construct($container, $container->get(Dispatcher::class), $container->get(ExceptionHandlerDispatcher::class), $container->get(ResponseEmitter::class));
    ...
    protected function init(string $method, string $path, array $options = []): ServerRequestInterface
    {
        ...
        if ($method === 'POST' && data_get($headers, 'Content-Type') === 'application/json') {
            $content = json_encode($json, JSON_THROW_ON_ERROR | JSON_UNESCAPED_UNICODE);
            $data = $json;
        }
